...
def analyze_object_size_distribution(label_dir, jpg_dir, target_size=640, output_dir=None):
    """
    分析目标检测数据集中对象尺寸的分布情况
    ...
    plt.tight_layout()
    save_path = os.path.join(output_dir, "object_size_distribution.png") if output_dir else "object_size_distribution.png"
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"目标尺寸分布已保存至: {save_path}")
    plt.show()
...
def analyze_image_histogram(jpg_dir, num_samples=500, output_dir=None):
    """
    分析目录下图像的颜色直方图分布
    ...
    if output_dir:
        save_path = os.path.join(output_dir, "color_histogram.png")
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        print(f"颜色直方图已保存至: {save_path}")
...
if __name__ == "__main__":
    # 输出目录
    output_dir = r"E:\Desktop\MAC-WIN\04 OpenCV\98_Practice\3DPrinterNozzleInspection\dataset_2\train"

    # 基本用法
    label_dir = r"E:\Desktop\MAC-WIN\04 OpenCV\98_Practice\3DPrinterNozzleInspection\dataset_2\train\labels"
    results = analyze_label_distribution(label_dir, output_dir=output_dir, plot_title='YOLO 标签分布', save_plot=True)
    jpg_path = r"E:\Desktop\MAC-WIN\04 OpenCV\98_Practice\3DPrinterNozzleInspection\dataset_2\train\images"
    txt_path = r"E:\Desktop\MAC-WIN\04 OpenCV\98_Practice\3DPrinterNozzleInspection\dataset_2\train\labels"
    result = analyze_object_size_distribution(txt_path, jpg_path, output_dir=output_dir)
    # print(result)

    analyze_image_histogram(jpg_path, output_dir=output_dir)
